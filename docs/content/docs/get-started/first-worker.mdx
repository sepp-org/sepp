...
- [Building a worker loop](/docs/workers/worker-loop): driving the reserve and ack loop yourself for production.
- [Idempotent workers](/docs/workers/idempotent-workers): making handlers safe to run more than once.
- [Graceful shutdown & draining](/docs/guides/graceful-shutdown): stopping workers cleanly on deploys.
