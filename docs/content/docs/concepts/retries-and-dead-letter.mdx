...
- **Retry** (`Default`): another attempt as long as attempts remain, timed by the queue's [retry policy](#backoff-and-delay). Immediate unless the queue configures a delay.
