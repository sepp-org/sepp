...
The first two arrive through a nack; the third comes from a worker that simply went away on the last attempt (see [leasing & redelivery](/docs/concepts/job-lifecycle#lease-expiry-redelivery-and-fencing)). Dead-lettered jobs are terminal and are never redelivered.
...
Draining is the programmatic path. The [admin UI](/docs/get-started/admin-ui) reads the same store without consuming it: it lists a queue's dead letters with their cause and last reason, and can requeue or delete the ones you select.

## See also

- [Job lifecycle & delivery](/docs/concepts/job-lifecycle): how lease expiry feeds the attempt counter and where dead-letter sits.
- [Dead-letter management](/docs/operations/dead-letter-management): operating the dead-letter store.
- [Processing jobs](/docs/workers#idempotent-handlers): handling poison jobs that keep failing.
