...
- **nack**: failure. By default the queue's [retry policy](/docs/concepts/retries-and-dead-letter#backoff-and-delay) times the retry, immediately `Ready` unless the queue configures a delay; with a retry-after delay it goes to `Scheduled`; as a permanent failure it is dead-lettered straight away.
