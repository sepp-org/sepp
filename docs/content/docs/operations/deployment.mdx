...
- **Config** goes in a ConfigMap mounted over `/etc/sepp/sepp.toml` or as `SEPP_` environment variables. Keep secrets (`auth.api_keys`, `admin.keys`) in a Secret, not the ConfigMap.
